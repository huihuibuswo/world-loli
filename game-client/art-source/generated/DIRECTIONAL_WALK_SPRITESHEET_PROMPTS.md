...
- 露娜当前右向候选图是已知临时例外：源文件仍使用独立的 right 文件名，导入时逐帧水平翻转；后续获得正确右向图后可直接替换源图并移除该例外。
- 每名角色、每个方向单独生成一张方形 PNG 源图；当前生成器实际输出 `1254x1254`，项目导入器会统一转换。
...
画面规格：
- 正方形 PNG，优先使用生成器原始最高分辨率；当前已验证 `1254x1254`。
...
> 露娜的 `npc-luna-walk-right-source.png` 当前保存独立侧面候选图，由导入脚本临时逐帧水平翻转。其他角色的左右方向均为独立生成资源，不做运行时镜像。
...
- [ ] 图片为方形 PNG；当前生成器的正常原图尺寸为 `1254x1254`，不要手动缩放。
...
```text
将正方形画布严格划分为四个等宽竖直区域。每个区域只能放置一个完整角色，角色水平居中，任何身体、头发、披风和配饰都不得跨越区域边界。
```
